 ...
 // ActionScript file for TrdXLSFileLoad.mxml
	...
	private function selectHandler(event:Event):void 
	{
		  ...
		  {
		  	mx.controls.Alert.show(this.parentApplication.xResourceManager.getKeyValue('trd.files.selectone'),"Invalid Selection");
			return;
		  }
	...
	public function submitFile():void
	{
		...
		requesturl = new URLRequest("trd/trdXLSLoadDispatch.action?method=init&unique="+new Date().getTime());
	...
	public function confirmSubmitFile():void {
		...
		trdFileLoadConfirm.request = reqObj;
		trdFileLoadConfirm.send();
	}
	...
	public function submitConfirmResult(event:ResultEvent):void{
		...
		if(rs.child("Errors").length()>0){
			...
			noOfMsgCreatedStr = this.parentApplication.xResourceManager.getKeyValue('trd.files.uploaded.complete') + noOfMsgCreated;
		}
	}
	...
	public function bindAll():void{
		...
		qh.resultHeader.text = this.parentApplication.xResourceManager.getKeyValue('trd.files.uploaded.userconf');
    ...
    */
    ...
    private function openPopup(event:MouseEvent):void {
		...
		panel.title = this.parentApplication.xResourceManager.getKeyValue('trd.files.label.excel.loading');
		panel.height = 47;
		panel.width = 200;
