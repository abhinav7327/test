...
//import com.nri.rui.core.controls.CustomChangeEvent;
	...
	public function doUserConfirmation(event:Event):void {	
	    ...
	    if (_mode == "cancel") {
	    	...
	    	//flag = validateCancel();
	    	if (flag) {
		    	requestObj.method = "confirmClose";
	     	}
	    }/*else if (_mode == "reopen") {
	    	flag = validateCancel();
	    	if (flag) {
	...
	private function visiblityControl():void{
			...
			if(_mode=="cancel" ){
	    		...
	    		cancelLabel.includeInLayout = false;
			...
			if(_mode=="cancel" || _mode=="amend" || _mode=="ENTRY" || _mode=="AMEND"){
	...
	private function OperationResult(event: ResultEvent) : void {
	     /* if(event.result != null){
	     	...
	     	if (event.result.swpEntryActionForm != null) {
	     		XenosAlert.info("dqwqwe");
	     		confirmInitXML = event.result.swpEntryActionForm as XML ;
	     	}
	    ...
	    if(rs.child("Errors").length() > 0) {
	        ...
	        errPage.showError(errorInfoList);
	    		...
	    		if(_mode=="cancel" ){
	    		 	...
	    		 	cancelLabel.includeInLayout = true;
	...
	public function doCancel(event:Event):void {
		var requestObj:Object = new Object();
	    if (_mode == "cancel") {	
			requestObj.method = "backToCancel";
	    }/* else if (_mode == "reopen") {	
