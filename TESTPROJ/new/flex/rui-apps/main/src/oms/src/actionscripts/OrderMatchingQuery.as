       ...
       */ 
       override public function preQueryInit():void{
	        var rndNo:Number= Math.random();
	  		super.getInitHttpService().url = "oms/orderMatching.action?method=initialExecute&mode="+mode+"&rnd=" + rndNo;        	
        }
        ...
        */ 
        override public function preResetQuery():void{
		       var rndNo:Number= Math.random();
		  	   super.getResetHttpService().url = "oms/orderMatching.action?method=resetQuery&mode="+mode+"&rnd=" + rndNo;        	
        }
    	...
    	override public function preQuery():void{
			setValidator();
            qh.resetPageNo();	
            super.getSubmitQueryHttpService().url = "oms/orderMatching.action?";  
    	 ...
    	 private function doSendForCxlForceMatch():void{ 
			...
			if(flag){
				if(cxlForceMatchPkList.length > 0){
					...
					XenosAlert.info(this.parentApplication.xResourceManager.getKeyValue('oms.order.matching.select.matched.order'));
				}
			}
    	 }
    ...
    */ 
     ...
     */
  	public function checkSelectToModify(item:Object,selectedFlag:Boolean):void {
    	addOrRemove(selectedFlag,item.recCount);
    }
    
    override    public function preGenerateXls():String{
    	var url : String =null;
	    if(mode == Globals.MODE_QUERY){
	    	url = "oms/orderMatching.action?method=generateXLS";
    	}
    	return url;
  	}
  	...
  	override public function preGeneratePdf():String{
    	var url : String =null;
    	if(mode == Globals.MODE_QUERY ){
			url = "oms/orderMatching.action?method=generatePDF";
    	}
    	return url;
  	}