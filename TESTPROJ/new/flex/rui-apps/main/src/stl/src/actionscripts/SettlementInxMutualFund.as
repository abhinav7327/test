    ...
    */
     private function initPageStart():void {            
        if (!initCompFlg) {    
            ...
            req.instructionScreenType = "mutualFundSendNew";
            req.SCREEN_KEY = 13018;
            req.dynaConstraintName = StlConstants.NARRATIVE_70E;
            initializeInxQuery.request = req;        
            initializeInxQuery.url = "stl/trxMaintenanceMutualFundQueryDispatch.action?method=initialExecute&rnd=" + rndNo;                    
    ...
    */   
     private function initPage(event: ResultEvent) : void {     	
        ...
        for (i = 0; i < tempColl.length; i++) {
		   if (XenosStringUtils.equals(tempColl[i].value, "MUTUAL_FUND")){
...
//                        qh1.recordCount.text = strStatistics;
      ...
      */  
      ...
      private function confirmHandler(event:ResultEvent):void {          
        ...
        if (null != event) {
          ...
          trxpop = TrxConfirmMutualFund(PopUpManager.createPopUp(this, TrxConfirmMutualFund, true));
        ...
        trxpop.title = this.parentApplication.xResourceManager.getKeyValue('stl.stlinxmutualfund.lbl.confirm');
    ...
    */
     private function generateXls():void {
         var url : String = "stl/trxMaintenanceMutualFundQueryDispatch.action?method=generateXLS";
    ...
    */
     private function generatePdf():void {
        var url : String = "stl/trxMaintenanceMutualFundQueryDispatch.action?method=generatePDF";
        var request:URLRequest = new URLRequest(url);
        request.method = URLRequestMethod.POST;
        // set menu pk in the request
      	var variables:URLVariables = new URLVariables();
    ...
    */
	...
	private function duplicateRecords():void{
		if(this.duplicatebtn.visible){
			...
			sPopup.title = this.parentApplication.xResourceManager.getKeyValue('stl.stlinx.lbl.duplicaterecords.mutfund');
			sPopup.addEventListener(CloseEvent.CLOSE,closeHandler,false,0,true);
			sPopup.width  = this.parentApplication.width - 100;
			sPopup.height = 400;
			PopUpManager.centerPopUp(sPopup);
			sPopup.moduleUrl = "assets/appl/stl/TrxDuplicateRecordsMutualFund.swf";
		}
	}
	 /**
	...
	private function openBulkTransmitUserConfirmation(event:ResultEvent):void {
        ...
        if(totalBulkTransmitRecords > 0) {
        	bulkTransmitInxMutualFund = BulkTransmitInxMutualFund(PopUpManager.createPopUp(this, BulkTransmitInxMutualFund, true));
			bulkTransmitInxMutualFund.title = this.parentApplication.xResourceManager.getKeyValue('stl.stlinx.lbl.bulk.transmit.mutfund.user.confirmation');
	        bulkTransmitInxMutualFund.addEventListener("OkSystemConfirm",handleOkSystemConfirm);
	        bulkTransmitInxMutualFund.width = this.parentApplication.width - 200;
	        bulkTransmitInxMutualFund.height = this.parentApplication.height - 200;
        	bulkTransmitInxMutualFund.totalEligibleRecordsFetched = totalBulkTransmitRecords;
	        bulkTransmitInxMutualFund.totalRptExpected = new Number(bulkTransmitList[0].countExpectedRpt);
	        PopUpManager.centerPopUp(bulkTransmitInxMutualFund);
        } else {
        	if(this.transmitAll != null){
        		this.transmitAll.enabled = true;
        	}
