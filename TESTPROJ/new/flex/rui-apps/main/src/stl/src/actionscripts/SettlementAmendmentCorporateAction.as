...
override public function postAmendResultInit(mapObj:Object): void {
	...
	if(mapObj!=null) {
		...
		if(errorArrCol.length>0) {
			...
			this.parentDocument.title = this.parentApplication.xResourceManager.getKeyValue('stl.amendment.title.corporateaction');
...
{        
    ...
    if(!errPage.isError()){
        this.parentDocument.title = this.parentApplication.xResourceManager.getKeyValue('stl.amendment.title.corporateaction')+" - "+this.parentApplication.xResourceManager.getKeyValue('stl.message.userconfirmation');
...
{
	//XenosAlert.info("******** "+mapObj["errorFlag"].toString());
    submitUserConfResult(mapObj);
    if(mapObj != null && mapObj["errorFlag"].toString() != "error"){
        this.parentDocument.title =this.parentApplication.xResourceManager.getKeyValue('stl.amendment.title.corporateaction')+" - "+this.parentApplication.xResourceManager.getKeyValue('stl.message.sysconfirmation');        
    }
}
override public function postCancelResultHandler(mapObj:Object): void{
    commonResult(mapObj);    
    if(!errPage.isError())
        this.parentDocument.title =this.parentApplication.xResourceManager.getKeyValue('stl.amendment.view.title.corporateaction');
...
//    XenosAlert.info("Populating request");
    ...
    reqObj['clientSettlementInfoVO.eventType'] = eventType;
    ...
    if(siVoInventoryAccountPk != null && siVoInventoryAccountPk != "") {
    	reqObj['settlementInfoVO.inventoryAccountPk'] = siVoInventoryAccountPk;
    }
    if(cashSiVoInventoryAccountPk != null && cashSiVoInventoryAccountPk != "") {
...
//    reqObj['cashStlDetail.dtcParticipantId'] = 
    ...
    reqObj['cashStlDetail.wayOfPayment'] = this.wayofpayment.text;
    ...
    if(inxForSuppress.includeInLayout == true){ // the value of inxTransmission needs to be sent on high priority basis.     
        reqObj['cashSettlementInfoVO.transmissionReqdFlag'] = this.inxForSuppress.selectedItem.value != null ? this.inxForSuppress.selectedItem.value : "";
    }
...
private function doBack():void {
	this.settlementMode.enabled = false;
	this.cashSettlementmode.enabled = false;
