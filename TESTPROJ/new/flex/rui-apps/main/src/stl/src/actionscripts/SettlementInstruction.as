    ...
    */
     private function initPageStart():void {            
        if (!initCompFlg) {    
            ...
            initializeInxQuery.url = "stl/trxMaintenanceQueryDispatch.action?method=initialExecute&rnd=" + rndNo;                    
...
//                        qh1.recordCount.text = strStatistics;
      ...
      */  
      ...
      private function confirmHandler(event:ResultEvent):void {          
        ...
        if (null != event) {
          ...
          trxpop = TrxConfirm(PopUpManager.createPopUp(this, TrxConfirm, true));
        ...
        trxpop.title = this.parentApplication.xResourceManager.getKeyValue('stl.stlinx.lbl.confirm');
    ...
    */
     private function generateXls():void {
         var url : String = "stl/trxMaintenanceQueryDispatch.action?method=generateXLS";
    ...
    */
     private function generatePdf():void {
        var url : String = "stl/trxMaintenanceQueryDispatch.action?method=generatePDF";
        var request:URLRequest = new URLRequest(url);
        request.method = URLRequestMethod.POST;
        // set menu pk in the request
      	var variables:URLVariables = new URLVariables();
    ...
    */
	...
	private function duplicateRecords():void{
		if(this.duplicatebtn.visible){
			...
			sPopup.title = this.parentApplication.xResourceManager.getKeyValue('stl.stlinx.lbl.duplicaterecords');
			sPopup.addEventListener(CloseEvent.CLOSE,closeHandler,false,0,true);
			sPopup.width  = this.parentApplication.width - 100;
			sPopup.height = 400;
			PopUpManager.centerPopUp(sPopup);
			sPopup.moduleUrl = "assets/appl/stl/TrxDuplicateRecords.swf";
		}
	}
	 /**
	...
	private function openBulkTransmitUserConfirmation(event:ResultEvent):void {
        ...
        if(totalBulkTransmitRecords > 0) {
        	bulkTransmitInx = BulkTransmitInx(PopUpManager.createPopUp(this, BulkTransmitInx, true));
			bulkTransmitInx.title = this.parentApplication.xResourceManager.getKeyValue('stl.stlinx.lbl.bulk.transmit.user.confirmation');
	        bulkTransmitInx.addEventListener("OkSystemConfirm",handleOkSystemConfirm);
	        bulkTransmitInx.width = this.parentApplication.width - 200;
	        bulkTransmitInx.height = this.parentApplication.height - 200;
        	bulkTransmitInx.totalEligibleRecordsFetched = totalBulkTransmitRecords;
	        bulkTransmitInx.totalRptExpected = new Number(bulkTransmitList[0].countExpectedRpt);
	        PopUpManager.centerPopUp(bulkTransmitInx);
        } else {
        	if(this.transmitAll != null){
        		this.transmitAll.enabled = true;
        	}
