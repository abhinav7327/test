...
private function validateSettlementAmendment():Boolean {
	...
	if(settleType=="FREE") {  //getObj('clientSettlementInfoVO.settlementType').value=="FREE"
		//FREE scenario
		if(isSuppressed == "Y") 
		{
...
private function handleSecurityAmendment():void {
	...
	if(isBrokerAccount != "Y" || settlementMode.selectedItem.value != "INTERNAL") { //getObj('isBrokerAccount').value != "Y" || getObj('securityStlDetail.settlementMode').value != "INTERNAL"
		if(secSSILayout == "DTC") {			
		}
		if(secSSILayout == "BONY") {
		}
