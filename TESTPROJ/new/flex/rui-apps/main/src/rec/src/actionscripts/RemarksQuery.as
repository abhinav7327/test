 ...
 import com.nri.rui.core.utils.ProcessResultUtil;   
    ...
    */
     private function initPageStart():void {            
        if (!initCompFlg) { 
            ...
            req.SCREEN_KEY = 11016;
            initializeRemarksQuery.request = req;       
            initializeRemarksQuery.url = "rec/recRemarksDispatch.action?method=initialExecute&rnd=" + rndNo;                    
    ...
    */   
     private function initPage(event: ResultEvent) : void {
        ...
        var sortField1Default:String = event.result.recRemarksActionForm.sortField1;
        var sortField2Default:String = event.result.recRemarksActionForm.sortField2;
        var sortField3Default:String = event.result.recRemarksActionForm.sortField3;
        var sortField4Default:String = event.result.recRemarksActionForm.sortField4;
		...
		if(event.result)    		
    		initColl = event.result.recRemarksActionForm.matchStatusList.item as ArrayCollection;
        ...
        if(null != event.result.recRemarksActionForm.sortFieldList1.item){
        	tempColl = new ArrayCollection();
        	tempColl.addItem({label:" ", value: " "});
        	selIndx = 0;
        	
        	if(event.result.recRemarksActionForm.sortFieldList1.item is ArrayCollection)
                initColl = event.result.recRemarksActionForm.sortFieldList1.item as ArrayCollection;
            else
                initColl.addItem(event.result.recRemarksActionForm.sortFieldList1.item);
        ...
        initColl.removeAll();
        if(null != event.result.recRemarksActionForm.sortFieldList2.item){
        	tempColl = new ArrayCollection();
        	tempColl.addItem({label:" ", value: " "});
        	selIndx = 0;
        
        	if(event.result.recRemarksActionForm.sortFieldList2.item is ArrayCollection)
                initColl = event.result.recRemarksActionForm.sortFieldList2.item as ArrayCollection;
            else
                initColl.addItem(event.result.recRemarksActionForm.sortFieldList2.item);
        ...
        initColl.removeAll();
        if(null != event.result.recRemarksActionForm.sortFieldList3.item){
        	tempColl = new ArrayCollection();
        	tempColl.addItem({label:" ", value: " "});
        	selIndx = 0;
        
        	if(event.result.recRemarksActionForm.sortFieldList3.item is ArrayCollection)
                initColl = event.result.recRemarksActionForm.sortFieldList3.item as ArrayCollection;
            else
                initColl.addItem(event.result.recRemarksActionForm.sortFieldList3.item);
        ...
        //6. Setting values of the SortField3
        initColl.removeAll();
        if(null != event.result.recRemarksActionForm.sortFieldList4.item){
        	tempColl = new ArrayCollection();
        	tempColl.addItem({label:" ", value: " "});
        	selIndx = 0;
        
        	if(event.result.recRemarksActionForm.sortFieldList4.item is ArrayCollection)
                initColl = event.result.recRemarksActionForm.sortFieldList4.item as ArrayCollection;
            else
                initColl.addItem(event.result.recRemarksActionForm.sortFieldList4.item);
    ...
    */ 
     private function doNext():void { 
        ...
        cashRemarksQueryRequestresult.request = reqObj;
        cashRemarksQueryRequestresult.send();
    } 
    /**
    ...
    */
    ...
    {  
        //Set the request parameters
         ...
         cashRemarksQueryRequestresult.request = requestObj; 
    ...
    */
 	...
 	private function populateBankAndBankAccount(event:ResultEvent):void{
 		...
 		/* if (null != event) {
 			if(null == event.result.recRemarksActionForm){ 
     			if(null == event.result.XenosErrors){ // i.e. No result but no Error found.
	            	   errPage.clearError(event); //clears the errors if any
	      			 }
	      		else{ // Must be error
	      		...
	      		errPage.clearError(event);	      	 	
            	queryResult = event.result.recRemarksActionForm as Object;      	
            	finContextList.removeAll();
    ...
    */
     private function generateXls():void {
        var url : String = "rec/recRemarksDispatch.action?method=generateXLS";
    ...
    */
     private function generatePdf():void {
        var url : String = "rec/recRemarksDispatch.action?method=generatePDF";
        var request:URLRequest = new URLRequest(url);
        request.method = URLRequestMethod.POST;
        // set menu pk in the request
      	var variables:URLVariables = new URLVariables();
