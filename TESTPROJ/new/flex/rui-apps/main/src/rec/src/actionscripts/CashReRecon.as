 ...
 import com.nri.rui.rec.validators.CashReReconValidator;
 import com.nri.rui.core.utils.ProcessResultUtil;
 ...
 import mx.utils.StringUtil;
    ...
    */
     private function initPageStart():void {      
        ...
        if (!initCompFlg) {    
            ...
            req.SCREEN_KEY = 10105;
            initializeCashReconQuery.request = req;      
            initializeCashReconQuery.url = "rec/cashReReconcile.action?method=initialExecute&rnd=" + rndNo;                    
            initializeCashReconQuery.send();        
        } else
    ...
    */   
     private function initPage(event: ResultEvent) : void {
        ...
        if(event.result){        	
        	if(event.result.recCashReReconcileActionForm.officeIdList.officeId is ArrayCollection){
    		initColl = event.result.recCashReReconcileActionForm.officeIdList.officeId as ArrayCollection;        		
        	}else{
        		initColl = new ArrayCollection();
        		initColl.addItem(event.result.recCashReReconcileActionForm.officeIdList.officeId);
        	}
        	if(event.result.recCashReReconcileActionForm.asOfDateFrom!= null) {
            dateStrFrom=event.result.recCashReReconcileActionForm.asOfDateFrom;
        	...
        	if(event.result.recCashReReconcileActionForm.asOfDateTo!= null) {
            dateStrTo=event.result.recCashReReconcileActionForm.asOfDateTo;
               // XenosAlert.info("from date :"+dateStr);
            if(dateStrTo != null)
     ...
     private function submitQuery():void {
        ...
        var myModel:Object={
                            reRecon:{
				                 ...
				                 securityCode:this.ccyBox.ccyText.text,
                                 accNo:this.stlAccForFundPopUp.accountNo.text          
                            }
                           }; 
        var cashReReconValidate:CashReReconValidator =new CashReReconValidator();
    ...
    */
    private function populateRequestParams():Object {
        ...
        var reqObj : Object = new Object();
         ...
         reqObj.ccy = ccyBox.ccyText.text;
         reqObj.SCREEN_KEY = 10114;
			...
			if(rs.child("row").length()>0) {
				...
				try {
	            	...
	            	summaryResult.refresh();
		           	qh.pdf.enabled = false;
	            	qh.xls.enabled = false;  
	            	this.selectAllBind = true;
	            	selectAllRecords(this.selectAllBind);
    ...
    public function checkSelectToModify(item:Object):void {
     ...
     public function showResult(event:Event):void {
		...
		if(conformSelectedResults.length > 0 ){
    		...
    		obj.chkBoxArray = chkBoxArray;
    		obj.SCREEN_KEY = 10106;
     ...
     public function LoadConfirmationOueryPage(event:ResultEvent):void {
				...
				sPopup.moduleUrl = "assets/appl/rec/RereconConfirmationPopup.swf";
