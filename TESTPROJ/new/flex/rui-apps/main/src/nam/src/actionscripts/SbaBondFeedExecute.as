...
// ActionScript file for Sba Feed Execute
	...
	import com.nri.rui.nam.validators.SbaBondFeedExecutionValidator;
	...
	private function initPage(event:ResultEvent):void
	{
		...
		this.fundCategory.dataProvider=tempColl;
		 ...
		 // Setting values of the issue currency
		initColl.removeAll();
		if (event.result.sbaFeedExecuteActionForm.issueCurrencyList.item != null)
		{
			if (event.result.sbaFeedExecuteActionForm.issueCurrencyList.item is ArrayCollection)
			{
				initColl=event.result.sbaFeedExecuteActionForm.issueCurrencyList.item as ArrayCollection;
			}
			else
			{
				initColl.addItem(event.result.sbaFeedExecuteActionForm.issueCurrencyList.item);
			}
		}
		...
		this.issueCurrency.dataProvider=tempColl;
	...
	private function resetExecute():void
	{
		var req:Object=new Object();
		req.SCREEN_KEY=13003;
		SBAExecuteResetRequest.request=req;
	...
	{
		var validateModel:Object={sbaBondFeed: 
		{
			officeId:this.officeId.text,
			fundCategory:this.fundCategory.text,
			issueCurrency:this.issueCurrency.text,
			originalDataSource:this.originalDataSource.text
		}};

		var sbaBondFeedExecutionValidator:SbaBondFeedExecutionValidator=new SbaBondFeedExecutionValidator();
		sbaBondFeedExecutionValidator.source=validateModel;
		sbaBondFeedExecutionValidator.property="sbaBondFeed";
	
		var validationResult:ValidationResultEvent=sbaBondFeedExecutionValidator.validate();
	...
	{
		sbaBondFeedEntry.selectedChild=entry;
	...
	private function fundCodeValidateResult(event:ResultEvent):void
	{
		...
		if (null != event)
		{
			...
			{	
				...
				fundCategoryCfm.text = this.fundCategory.selectedItem != null ? StringUtil.trim(this.fundCategory.selectedItem.label) : "";
				issueCurrencyCfm.text = this.issueCurrency.selectedItem != null ? StringUtil.trim(this.issueCurrency.selectedItem.label) : "";
				if (XenosStringUtils.equals('Advisory', this.fundCategory.selectedItem.value))
				{
				...
				submitCfm.setFocus();
				sbaBondFeedEntry.selectedChild = uConfirm;
	 ...
	 private function feedExecuteResult(event:ResultEvent):void
	 {
		...
		if (null != event)
		{
			...
			else
			{
				...
				ok.setFocus();
				sbaBondFeedEntry.selectedChild = sConfirm;
	...
	{	
		...
		reqObj.issueCurrency=(this.issueCurrency.selectedItem != null ? this.issueCurrency.selectedItem.value : "");
