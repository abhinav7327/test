...
// ActionScript file for FAM Transaction Excel Upload Screen.
...
public function bindAll():void
{
	...
	qh.resultHeader.text=this.parentApplication.xResourceManager.getKeyValue('fam.excel.upload.txn.label.user.confirm');
...
public function confirmSubmitFile():void
{
	...
	transactionFileLoadConfirm.request=reqObj;
	transactionFileLoadConfirm.send();
}
...
public function initCommandForm(event:ResultEvent):void
{
	...
	commandForm=event.result.transactionUploadCommandForm;
	commandFormId=commandForm.commandFormId;
	url="fam/transactionUpload.spring?method=initPageForXLSUpLoad&commandFormId=" + commandFormId;
}

public function initOnPageLoad():void
{

	initializeTransactionUpload.url="fam/transactionUpload.spring?method=initPage";
	initializeTransactionUpload.send();
}
...
public function preset(event:ResultEvent):void
{
	...
	resultForm=result.famTxns;
	if (result.child("famTxns").length() > 0)
	{
		...
		try
		{
			...
			errPage.clearError(event);
			summaryResult.removeAll();
			for each (var rec:XML in resultForm.famTxn)
			{
	...
	{
		XenosAlert.info(this.parentApplication.xResourceManager.getKeyValue('fam.excel.upload.txn.norecordfound'));
		summaryResult.removeAll(); // clear previous data if any as there is no result now.
		summaryResult.refresh();//refresh summary result data provider
...
public function submitConfirmResult(event:ResultEvent):void
{
	...
	else
	{
		...
		var noOfMsgCreated:String=event.result.transactionUploadCommandForm.noOfMessageProcessed;
		noOfMsgCreatedStr=this.parentApplication.xResourceManager.getKeyValue('fam.excel.upload.txn.label.uploadcomplete') + noOfMsgCreated;
	}
}
...
public function submitFile():void
{
	...
	requesturl=new URLRequest("fam/transactionUpload.spring?method=initPageForXLSUpLoad");
...
private function openPopup(event:MouseEvent):void
{
	...
	panel.title=this.parentApplication.xResourceManager.getKeyValue('fam.excel.upload.txn.label.loading');
	panel.height=47;
	panel.width=200;
