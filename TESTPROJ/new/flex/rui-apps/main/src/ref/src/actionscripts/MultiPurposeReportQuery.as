...
[Bindable]private var requestHiddenParamStr : String = "";
      ...
      override public function preQueryInit():void{
        ...
        super.getInitHttpService().url = "ref/multiPurposeRptQryDispatch.action?method=initialExecute";
        var req : Object = new Object();
        req.SCREEN_KEY = "12156";
    ...
    */
	private function isValidFormField(alertString:String):Boolean {
		...
		if(null == this.sqlName.selectedItem || XenosStringUtils.isBlank(this.sqlName.selectedItem.value.toString())){
			XenosAlert.error(this.parentApplication.xResourceManager.getKeyValue('ref.multipurpose.query.msg.alert.sqlname'));
			return false;
		}
		for(var row:int = 0; row < rowCount; row++){
			...
			if(XenosStringUtils.equals(compName[itemCount],"account"+itemCount)){
				...
				if(DateUtils.isValidDate(ti.text)){					
					...
					XenosAlert.error(this.parentApplication.xResourceManager.getKeyValue('ref.error.illegal.date'));
					return false;
				}
			} else if(XenosStringUtils.equals(compName[itemCount],"combo"+itemCount)){
				valueFromUi = ti.selectedItem.value;
			} else if(XenosStringUtils.equals(compName[itemCount],"list"+itemCount)){
				valueFromUi ="";
				for each(var item:Object in ti.selectedItems){
                   valueFromUi = item.value +"|" +valueFromUi ;
                }
                valueFromUi = valueFromUi.slice(0,-1);	
			...
			else (!(XenosStringUtils.equals(compName[itemCount],"checkbox"+itemCount)
    ...
    private function LoadSummaryPage(event:ResultEvent):void{
			    		...
			    		errPage.showError(errorInfoList);//Display the error
	       				...
	       				if(rsltView.child("item").length()>0){
						 	...
						 	for each (var itm : Object in headerNames){
						 		...
						 		if(dataGridColumn.getStyle("textAlign")=="right")
						 		{
						 			dataGridColumn.sortCompareFunction = sortUtil.sortNumeric;
						 		}
       ...
       // This method is used to hide the PDF Report download Image.
        ...
        private function commonInit(mapObj:Object):void{
	    	...
	    	if(initcol.length <= 1)
	    	{
	    		XenosAlert.error(this.parentApplication.xResourceManager.getKeyValue('ref.multipurpose.query.msg.alert.nosql'));
	    	}
         ...
         override public function preResetQuery():void{
		  		...
		  		super.getResetHttpService().url = "ref/multiPurposeRptQryDispatch.action?method=reset";        	
        }
        
         override public function preGenerateXls():String{
        	 var url : String = "ref/multiPurposeRptQryDispatch.action?method=generateXLS";	
		     return url;
         }  
        ...
        if (null != event && (rs.child("parameterEmpty") == "false" || rs.child("parameterEmpty").length() <= 0)) {
	    	if(rs.child("Errors").length()>0) {	    		
	    		...
	    		try {
					...
					else if(XenosStringUtils.equals(uitypeFirst,"List")){
						var list:List = List(rowColumn.getChildren()[0]);
    	...
    	if (rs != null) {
    		if(rs.child("uIconfigListForRUI").length()>0) {
    			try {
    				...
    				var dropDownData:ArrayCollection;
    				...
    				for each ( var rec:XML in rs.uIconfigListForRUI.UIconfigListForRUI) {
						...
						if(XenosStringUtils.isBlank(uitype) != true){							
							...
							if(XenosStringUtils.equals(uitype,"DROPDOWN")){
								dropDownData = new ArrayCollection();
							...
							if(XenosStringUtils.equals(uitype,"LIST")){
								listData = new ArrayCollection();
								if(XenosStringUtils.equals(blankRequiredFlag,"true"))																
									listData.addItem({label:" ", value:" "});						
				
								if(rec.constraintListRUI != null) {
									for each(var xmlObj2:Object in rec.constraintListRUI.item){
										listData.addItem(xmlObj2);
									}									
								}	
										
							}
		    				//add the items into rows & not include for ui type
		    				if(DynamicDisplayOfData(gridRow, labelKey, uitype,caseOfText,dropDownData,listData,isOptional,defaultValue) == false){
								componentCount--;
		    				}
							i++;
							componentCount++;
    				...
    				XenosAlert.error(this.parentApplication.xResourceManager.getKeyValue('ref.error.generateui'));
    			}
    		}
    	}
    }    	
  ...
  /**
    ...
    public function DynamicDisplayOfData(gridRow:GridRow, 
		...
		if(XenosStringUtils.equals(uitype,"DROPDOWN")){
			...
			//gridItem.addChild(myComboBox);				           							            
			gridRow.addChild(gridItem);
			flag = true;
		}
		if(XenosStringUtils.equals(uitype,"TREE")){
			gridItem = new GridItem();
			var comboBox:ComboBox = new ComboBox();
			gridItem.percentWidth = 30;		
			gridItem.horizontalScrollPolicy = "off";	
		...
		if(XenosStringUtils.equals(uitype,"LIST")){
			...
			//add name
			list.name = "list"+compCount;
			compName[compCount] = list.name;        
			compCount++;
			
			list.allowMultipleSelection=true;
			list.dataProvider = listData;
			list.selectedIndex = getIndex(listData,defaultValue);
		    list.width = 300;                     
			gridItem.addChild(list);				           
