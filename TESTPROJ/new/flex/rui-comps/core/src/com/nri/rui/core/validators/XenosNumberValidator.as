...
package com.nri.rui.core.validators
{    
    ...
    /**
     *  The XenosNumberValidator class ensures that a String represents a valid number(accepts BTM format also).
     ...
     *  @mxml
     *
     *  <p>The <code>&lt;XenosNumberValidator&gt;</code> tag
     ...
     *  
     */
    public class XenosNumberValidator extends Validator
    {    	
    	 ...
    	 *
    	 *  @param validator The XenosNumberValidator instance.
    	 *
         ...
         */
        public static function validateNumber(validator:XenosNumberValidator,
    										  value:Object,
    										  baseField:String):Array
        {
            ...
            var input:String = String( value.toString().replace(/\,/g,"")).toUpperCase();
            ...
            var BMT_CHARS:String = "BMT";
            var XENOS_DIGITS:String = DECIMAL_DIGITS + BMT_CHARS ;
            ...
            //check for the unique single occurance of BMT
            if((input.indexOf("B") != -1 && "B" != input.charAt(len-1)) ||
               (input.indexOf("M") != -1 && "M" != input.charAt(len-1)) ||
               (input.indexOf("T") != -1 && "T" != input.charAt(len-1))){
                results.push(new ValidationResult(
    				true, baseField, "invalidFormatChar",
    				validator.invalidFormatCharsError));    				
    			return results;
            ...
            var validChars:String = XENOS_DIGITS + "-" +
            ...
            if (decimalSeparatorIndex != -1)
            {
                ...
                for (i = decimalSeparatorIndex + 1; i < len; i++)
                {
                    ...
                    if (XENOS_DIGITS.indexOf(input.charAt(i)) == -1)
                    {
                        results.push(new ValidationResult(
    						true, baseField, "invalidChar",
    						validator.invalidCharError));
    				...
    				// if domain is int.
                    if (domain == "int" && (input.charAt(i) != "0" && BMT_CHARS.indexOf(input.charAt(i))==-1))
                    {
                        results.push(new ValidationResult(
            ...
            for (i = 1; i < end; i++)
            {
                ...
                if (c == thousandsSeparator)
                {
                    ...
                    {
                        if ((end - i != 4 &&
    						 (input.charAt(i + 4) != thousandsSeparator && BMT_CHARS.indexOf(input.charAt(i+4))==-1)) ||
                ...
                else if (XENOS_DIGITS.indexOf(c) == -1)
                {
                    results.push(new ValidationResult(
    					true, baseField,"invalidChar",
    					validator.invalidCharError));    					
            ...
            //Setting default values to the minValue and maxValue if they are already not supplied
            if (isNaN(maxValue)){
                maxValue = Number("99999999999999");                
            }
            if (isNaN(minValue)){               
                minValue = Number("0");
            }
            ...
            if (!isNaN(minValue) || !isNaN(maxValue))
            {
                ...
                // Check bounds
                var strInput:String = getNumericValue(input);
    	 ...
    	 *  Constructor.
    	 */
    	public function XenosNumberValidator()
    	{
    		super();
    	}
        ...
        override protected function doValidation(value:Object):Array
        {
    		...
    		if (results.length > 0 || ((val.length == 0) && !required))
    		    ...
    		    return XenosNumberValidator.validateNumber(this, value, null);
        }
        /**
         * Get numeric value of the given field.
         */
        private static function getNumericValue(value:Object):String {   
        ...
        */
        private static function getValidNumber(number:String, pos:int):String  {
	     ...
	     */
	     public function handleNumericField(numberFormatter:XenosNumberFormatter):Boolean{
