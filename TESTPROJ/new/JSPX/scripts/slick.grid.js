...
(function($) {
    ...
    function SlickGrid(container,data,columnDefs,options) {
        ...
        var $headerScroller;
        var $headers;
        var $headerRow, $headerRowScroller,$spanHeaders,$masterHeaders;
        ...
        var sortAsc = true;
		var spanColumns = [];
		var masterHeaderColumns = [];
		var hasMasterHeader = false;
        ...
        function initializeColumns() {
			...
			for (var i = 0; i < columnDefs.length; i++) {
				//don't consider is masterColumn
		...
		function seperateMasterHeader() {
			if(options.hasMasterHeader){
			//seperate master header column defs from source column definitions
			var tempColumnDef = [];
				for (var i = 0; i < columnDefs.length; i++) {
					if(columnDefs[i].masterColumn){
						
						masterHeaderColumns.push(columnDefs[i]);
					}else{
						tempColumnDef.push(columnDefs[i]);
					}
				}
        ...
        function init() {
			...
			//seperate master header if any
			seperateMasterHeader();
			initializeColumns();
			...
			$masterHeaders = $("<div class='slick-header-columns' style='left:-1000px' />").appendTo($headerScroller);
        ...
        function createColumnHeaders() {
            ...
            for (i = 0; i < columns.length; i++) {
                ...
                var header = $("<div  title=\""+m.name+"\" class='ui-state-default slick-header-column' id='" + uid + m.id + "' />")
			...
			if(!(sortColumnId === undefined)){
				var column = columns[getColumnIndex(sortColumnId)];
				if(!(column === undefined)){
					trigger(self.onSort, {sortCol:column,sortAsc:sortAsc}, null);
				}
			}
            setupColumnResize();
			...
			createSpanHeaderRow();
			createMasterHeaderRow();
        }
		
		//Method used to create master header over Spanned header row
		function createMasterHeaderRow(){
			if(options.hasMasterHeader){
				$masterHeaders.empty();
				for (var i = 0; i < masterHeaderColumns.length; i++) {
				m = masterHeaderColumns[i];
				var index = spanColumns[m.start].startIndex;
				var width = 0;
				 do {
					width += $($($headers).children().get(index)).width();	
					width += headerColumnWidthDiff;
					index++;
                } while (index <=spanColumns[m.end].endIndex);
				 width -= headerColumnWidthDiff;
				var header = $("<div class='ui-state-default slick-header-column' id='" + uid + m.id + "' />")
                 ...
                 .data("column", m)
                 .addClass(m.headerCssClass || "masterHeaderAlignWithBorder")
                 .appendTo($masterHeaders);
				}
				$($masterHeaders).width($($headers).width());
			}else{
				$masterHeaders.remove();
                return;
			}
		}
		...
		function calculateTextWidth(el){
			...
			if(html_org.indexOf('span')!=-1){
				$('#temp-width-check').html($(el).find('span:first').text());
			}else{
				$('#temp-width-check').html($(el).text());
			}
			var width = $('#temp-width-check').width();
			return width;
		}

		function setupAdjustColumns(){
			if(options.autoAdjustColumnWidth){
				$headers.dblclick(function(e) {
					$('body').append('<div class="slick-cell"><span id="temp-width-check" style="width:auto;"></span></div>');
					if (!$(e.target).hasClass("slick-resizable-handle")) {
						return;
					}
					var $col = $(e.target).closest(".slick-header-column");
					...
					var columnIndex = getColumnIndex($col.data("fieldId"));
					var maxDataWidth = 0;
					var columnCells = $('.l'+columnIndex,$container);
					$.each(columnCells,function(index,el){
						var dataWidth = calculateTextWidth(el);
						if(maxDataWidth < dataWidth)
							maxDataWidth = dataWidth;
					});
					if(maxDataWidth != 0){					
						columns[columnIndex].width = maxDataWidth+12;
					}else{
						columns[columnIndex].width = 40;
					}
					self.setColumns(columns,true);
					$('#temp-width-check').closest('.slick-cell').remove();
				});
			}
		}
        ...
        function setupColumnReorder() {
            $headers.sortable({
                ...
                stop: function(e) {
					...
					//Modified for xenos Grid purpose, commented lines of code has been moved to the method setColumns
        ...
        function applyColumnWidths() {
            ...
            //Fix for IE, handling negative rowWidth in case of IE.
            if(rowWidth<0 &&  $.browser.msie){
                rowWidth=0;
            }
            rule.style.width = rowWidth + "px"; //does not work in ie when rowWidth is negative.
        }
        ...
        function setColumns(columnDefinitions,forceCall) {
            ...
            resizeAndRender();
            handleScroll();
			trigger(self.onColumnsReordered, {forceCall:forceCall});		
        }
        ...
        function appendRowHtml(stringArray, row) {
			...
			/* Added by xenos: Begin */
			if(typeof d !== 'undefined'){
				if(d.status && d.status == "CANCEL" || d.status == "CXL"){
					var bgColor = row % 2 == 1 ? options.cxlColorOdd:options.cxlColorEven;
				}else if(d.settlementStatus && d.settlementStatus == "FAIL" ){
					var bgColor = row % 2 == 1 ? options.failColorOdd:options.failColorEven;
				}
				else if ( typeof options.resolveBgColor === 'function' ) {
					var bgColor = options.resolveBgColor(getDataItem(row));
					if ( bgColor === '' )
						bgColor = row % 2 == 1 ? options.oddRowColor:options.evenRowColor;
				}
				
				else{
					var bgColor = row % 2 == 1 ? options.oddRowColor:options.evenRowColor;
				}
			}else{
				var bgColor = row % 2 == 1 ? options.oddRowColor:options.evenRowColor;
			}
			...
			/* Edited: (xenos) info:Adding the bgcolor param in the style*/
            stringArray.push("<div class='ui-widget-content " + rowCss + "' row='" + row + "' style='background-color:"+ bgColor +";top:" + (options.rowHeight*row-offset) + "px'>");
            ...
            for (var i=0, cols=columns.length; i<cols; i++) {
                ...
                stringArray.push("<div class='" + cellCss + "' "+styleStr+">");
				
                // if there is a corresponding row (if not, this is the Add New row or this data hasn't been loaded yet)
                if (d) {
					var isSelectorRequired = options.isSelectorRequired===undefined ? true : options.isSelectorRequired;
                    if(options.isPopUpQuery && i==0 && isSelectorRequired){
						var selectHeadermarkUp = '<span class="select-pop-query" row='+ row +'></span>';
						stringArray.push(selectHeadermarkUp);
					}else{
        ...
        function getViewportHeight() {
            return parseFloat($.css($container[0], "height", true)) -
                ...
                (headerGrouping ? options.headerRowHeight : 0) -
				(hasMasterHeader ? options.headerRowHeight : 0);
        }
        ...
        function render() {
            ...
            if(heightAdjustFlag){
	            if(options.hasMasterHeader){
					var modHeight = $('.xenos-grid').height();
					$('.xenos-grid').height(modHeight+50);
					heightAdjustFlag = false;
				}
	        }
        }
