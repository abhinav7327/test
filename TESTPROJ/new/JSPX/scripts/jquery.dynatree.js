...
//		this.tree.logDebug("%s.addChild(%o, %o)", this, obj, beforeNode);
		...
		for (var i=0, l=obj.length; i<l; i++) {
			...
			// Anjan
			if (data.menuAction) {
				if (data.menuAction.action) {
					data.title=data.menuAction.actionName;
					data.select = this.tree.options.wizard.menuActionSelection[data.menuAction.subActionId];
					data.expand = this.tree.options.wizard.menuActionSelection[data.menuAction.subActionId];
					data.addClass = 'xenos-action-leaf';
				} else {
					data.title=data.menuAction.menuName;
					data.select = this.tree.options.wizard.menuActionSelection[data.menuAction.menuPk];
					data.expand = this.tree.options.wizard.menuActionSelection[data.menuAction.menuPk];
				}
			} else {
				data.expand = true;
				data.title = xenos.title.menuLabel;
			}

			if (!data.select && this.tree.options.wizard.options.mode != 'EDIT') {
				continue;
			}
