...
//Date validation for American style Entry MMDDYYYY and display DD-MMM-YYYY
function dateValidation(inpDate)
{	var mm,dd,yy;
	var dateVal = inpDate;
	var valid="AaBbCcDdEeFfGgHhIiJjLlMmNnOoPpRrSsTtUuVvYy0123456789-";
	
	if	(dateVal.length == 0 ) return "";
	if	(dateVal.length > 11)
		{	alert ("FORMAT should be MMDDYYYY or MMDDYY");
	...
	for (var i=0; i<dateVal.length; i++)
		{	if	(valid.indexOf(dateVal.charAt(i)) == -1)
				{	alert("INVALID FORMAT. Input contains unacceptable characters");
					return "";
				}
		}
...
//Check for length of date field = 10 -------------------------------------------------------
		...
		{	if	(isNaN(dateVal))
				{	if ((dateVal.charAt(2) == "-") && (dateVal.charAt(5) == "-"))
						{	mm = dateVal.substring(0,2);
							dd = dateVal.substring(3,5);
